...
while capture.isOpened():
    ...
    if ret:
        ...
        for cnt in array_contour:
            ...
            area_min = 2000
            area_max = 50000
            if np.size(approx, 0) == 4 and cv2.isContourConvex(approx) and area_min <= cv2.contourArea(
                    approx) <= area_max:
                point_list = []
        ...
        # 원 영상의 frame rate 를 그대로 사용하기 위한 방법. int(1000 / capture.get(cv2.CAP_PROP_FPS))
